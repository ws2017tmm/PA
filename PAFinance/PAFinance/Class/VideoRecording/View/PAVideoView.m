...
/** 视频播放 */
@property (nonatomic,strong) PAAVPlayer *player;
    ...
    if ([self.captureMovieFileOutput isRecording]) {
        ...
        if (self.seconds > 0) {
            ...
            if (self.paSeconds - self.seconds >= TimeMax && !self.isVideo) {
                ...
                // ?
//                self.progressView.timeMax = self.PASeconds;
            }
            [self performSelector:@selector(onStartTranscribe:) withObject:fileURL afterDelay:1.0];
        } else {
            if ([self.captureMovieFileOutput isRecording]) {
...
- (PAProgressView *)progressView {
    if (!_progressView) {
        _progressView = [PAProgressView new];
...
-(AVCaptureDevice *)getCameraDeviceWithPosition:(AVCaptureDevicePosition )position{
    if (@available(iOS 10.0, *)) {
        ...
        AVCaptureDeviceDiscoverySession *discoverySession = [AVCaptureDeviceDiscoverySession discoverySessionWithDeviceTypes:@[type] mediaType:AVMediaTypeVideo position:position];
        // iOS10 之后
...
-(void)tapScreen:(UITapGestureRecognizer *)tapGesture{
    if ([self.session isRunning]) {
        ...
        /*
        AVCaptureFocusModeLocked               锁定当前镜头位置
        AVCaptureFocusModeAutoFocus            镜头自动调焦
        AVCaptureFocusModeContinuousAutoFocus  镜头连续自动调焦
         */
        /*
